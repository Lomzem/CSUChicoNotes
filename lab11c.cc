...
int main(void) {
    // all_sockets stores all active sockets. Any socket connected to the server
    // should be included in the set. A socket that disconnects should be
    // removed from the set. The server's main socket should always remain in
    // the set.
    fd_set all_sockets;
    FD_ZERO(&all_sockets);
    // call_set is a temporary used for each select call. Sockets will get
    // removed from the set by select to indicate each socket's availability.
    fd_set call_set;
    FD_ZERO(&call_set);
    ...
    // accept() setup taken from Beej Guide
    struct sockaddr_storage their_addr;
    socklen_t addr_size;
    int new_fd;

    // buffer to hold stuff we got from recv()
    ...
    while (1) {
        ...
        for (int s = 3; s <= max_socket; ++s) {
            ...
            if (s == listen_socket) {
                ...
                new_fd = accept(listen_socket, (struct sockaddr*)&their_addr,
                                &addr_size);
                ...
                // add new socket to all_sockets set
                FD_SET(new_fd, &all_sockets);

                // print a message with new socket number
                std::cout << "Socket " << new_fd << "connected\n";
            }
            ...
            // A connected socket is ready
                ...
                // end up here as well.
                int resp = recv(s, buf, RECV_BUFFER_SIZE, 0);
                if (resp < 0) {
                    std::cerr << "ERROR: Unable to recv from " << s << "\n";
                    close(s);

                    std::cout << "Socket" << s << " closed\n";

                    // just remove socket from set instead of ending program because
                    // there may still be valid sockets left in set
                    FD_CLR(s, &all_sockets);
                }

                if (resp == 0) {
                    // other client closed connection
                    close(s);
                    std::cout << "Socket" << s << " closed\n";

                    // remove socket from all_sockets set
                    FD_CLR(s, &all_sockets);
                }

                if (resp > 0) {
                    // client sent data, print it to stdout
                    std::cout << "Socket " << s << " sent: ";
                    std::cout.write(buf, resp);
                    std::cout << "\n";
                }
            }
        }
